...
class RedisStreamAdapter:
    ...
    async def read_messages_by_range(
        ...
        Args:
            start_id: Start message ID (str or bytes)
            end_id: End message ID (str or bytes)
        ...
        """
        try:
            # Ensure stream_key, start_id, and end_id are bytes
            stream_key_bytes = self.stream_key.encode('utf-8') if isinstance(self.stream_key, str) else self.stream_key
            start_id_bytes = start_id.encode('utf-8') if isinstance(start_id, str) else start_id
            end_id_bytes = end_id.encode('utf-8') if isinstance(end_id, str) else end_id
            ...
            if messages:
                for message_id, fields in messages:
                    ...
                    # Decode field keys and values safely
                    decoded_fields = {}
            ...
            logger.info(f"Read {len(result)} messages from range {start_id} to {end_id} in stream {self.stream_key}")
            return result
